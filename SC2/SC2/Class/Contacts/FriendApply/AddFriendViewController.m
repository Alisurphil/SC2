...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self didBuddyExist:buddyName]) {
        NSString *message = [NSString stringWithFormat:( @"'%@'已经是你的好友!"), buddyName];
...
#warning 以下代码为测试代码，默认用户体系中有一个符合要求的同名用户
        ...
        if ([_textField.text isEqualToString:loginUsername]) {
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:(@"提示") message:(@"不能添加自己为好友") delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
            [alertView show];
        ...
        if (applyArray && [applyArray count] > 0) {
            for (ApplyEntity *entity in applyArray) {
                ...
                if (!isGroup && [entity.applicantUsername isEqualToString:_textField.text]) {
                    ...
                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:(@"提示") message:str delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
                    [alertView show];
